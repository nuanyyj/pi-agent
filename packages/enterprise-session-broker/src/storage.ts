...
import type { EnterpriseTransaction } from "./db.js";
...
export class BrokeredSessionStorage
  implements SessionStorage<EnterpriseSessionMetadata>
{
  private readonly broker: SessionBroker;
  private readonly metadata: EnterpriseSessionMetadata;
  ...
  async getMetadata(): Promise<EnterpriseSessionMetadata> {
  ...
  private reconcileVersion(durableVersion: number, durableLeafId: string | null): void {
...
function conflictError(sessionId: string, currentVersion: number): SessionError {
  return new SessionError(
    "invalid_session",
    `Version conflict on session ${sessionId}: expected ${currentVersion}`,
  );
}
 ...
 */
function resolveLeafId(entry: SessionTreeEntry): string | undefined {
  if (entry.type === "leaf") return entry.targetId ?? undefined;
