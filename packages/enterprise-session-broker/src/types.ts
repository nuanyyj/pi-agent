...
export interface SessionBroker {
  ...
  appendAndAdvance(input: AppendAndAdvanceInput): Promise<BrokerResult<{ version: number; entryId: string }>>;
  moveLeaf(input: MoveLeafInput): Promise<BrokerResult<{ version: number; leafId: string | null }>>;
  forkSession(
