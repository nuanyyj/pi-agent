...
import { createBrokeredHarness, } from "./brokered-runtime.js";
import { createEnterpriseDatabase } from "@pi-web/enterprise-session-broker";
// ── Model resolution ───────────────────────────────────────────────────
function resolveModel(models, provider, modelId) {
    models.refresh(provider).catch(() => { });
...
// ── PG event writer ────────────────────────────────────────────────────
export function createPgEventWriter(db, runId) {
    let seq = 0;
    ...
    return {
        writeEvent(type, data) {
            ...
            pendingWrites = pendingWrites.then(async () => {
                await db.query(`insert into enterprise_run_events (run_id, seq, event_type, event_data) values ($1, $2, $3, $4)`, [runId, eventSeq, type, JSON.stringify(data)]);
        ...
        async updateRunStatus(status, patch) {
            try {
                const sets = [`status = $1`];
                const params = [status];
                let idx = 2;
...
export async function executeRun(options) {
    ...
    try {
        ...
        const harnessResult = await createBrokeredHarness({
            db,
            envelope,
            models,
            model,
...
export async function runFromEnvelopePath(envelopePath, pgUrl, runId, signal) {
